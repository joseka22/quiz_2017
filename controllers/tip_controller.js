...
exports.load = function (req, res, next, tipId) {
    ...
    models.Tip.findById(tipId, {
        include: [
            {model: models.User, as: 'Author'} //incluyo tip.Author
            ]
    })
    .then(function (tip) {
        if (tip) {
...
exports.create = function (req, res, next) {
    ...
    //var authorId = req.session.user && req.session.user.id || 0; //para añadir el authorId

    var tip = models.Tip.build(
        {
            text: req.body.text,
            QuizId: req.quiz.id,
            AuthorId: req.session.user.id       //añado  authorId
        });

    tip.save({fields: ["text", "QuizId", "AuthorId"]}) //guardo
    .then(function (tip) {
